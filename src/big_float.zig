...
pub var internal_allocator: Allocator = undefined;
...
pub fn toStringFlat(
    ...
    max_size: usize,
    val: *const c.mpf_t,
) (error{too_big} || Allocator.Error)![]u8 {
    ...
    const out: []u8 = try alloc.alloc(u8, digits.len + needed_padding);
...
fn gmpAlloc(size: usize) callconv(.c) ?*anyopaque {
    return (internal_allocator.alloc(u8, size) catch std.debug.panic("GMP allocation failed!", .{})).ptr;
}
...
fn gmpRealloc(ptr: ?*anyopaque, old_size: usize, new_size: usize) callconv(.c) ?*anyopaque {
    ...
    return (internal_allocator.remap(old_slice, new_size) orelse blk: {
        const min_size = @min(old_size, new_size);
        const new_slice: []u8 = internal_allocator.alloc(u8, new_size) catch std.debug.panic("GMP allocation failed!", .{});
        @memcpy(new_slice[0..min_size], old_slice[0..min_size]);
        internal_allocator.free(old_slice);
        break :blk new_slice;
    }).ptr;
}
...
test "stringTest" {
    ...
    defer c.mpf_clear(&a);
    const a_str = try toStringFlat(std.testing.allocator, 19, 1024, &a);
    defer internal_allocator.free(a_str);
...
test toStringFlat {
    ...
    defer c.mpf_clear(&a);
    const a_str = try toStringFlat(std.testing.allocator, 5, 1024, &a);
    defer internal_allocator.free(a_str);
    try std.testing.expectEqualStrings("0.012872", a_str);

    var b = stringInit("12871623");
    defer c.mpf_clear(&b);
    const b_str = try toStringFlat(std.testing.allocator, 3, 1024, &b);
    defer internal_allocator.free(b_str);
    try std.testing.expectEqualStrings("12900000", b_str);

    var d = stringInit("2034.986");
    defer c.mpf_clear(&d);
    const d_str = try toStringFlat(std.testing.allocator, 6, 1024, &d);
    defer internal_allocator.free(d_str);
    try std.testing.expectEqualStrings("2034.99", d_str);

    // test trailing zeros
    var a_z = stringInit("0.012871");
    defer c.mpf_clear(&a_z);
    const a_z_str = try toStringFlat(std.testing.allocator, 9, 1024, &a_z);
    defer internal_allocator.free(a_z_str);
    try std.testing.expectEqualStrings("0.012871", a_z_str);

    var b_z = stringInit("1287");
    defer c.mpf_clear(&b_z);
    const b_z_str = try toStringFlat(std.testing.allocator, 8, 1024, &b_z);
    defer internal_allocator.free(b_z_str);
    try std.testing.expectEqualStrings("1287", b_z_str);

    var d_z = stringInit("2034.1996");
    defer c.mpf_clear(&d_z);
    const d_z_str = try toStringFlat(std.testing.allocator, 7, 1024, &d_z);
    defer internal_allocator.free(d_z_str);
