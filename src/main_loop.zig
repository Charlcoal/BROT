...
fn renderedBufferResolve(io: std.Io) !void {
    ...
    if (common.remapping_buffer) {
        ...
        for (common.back_r2c_offset[0..]) |*offset| {
            ...
            if (@abs(offset.y) * @as(f64, @floatFromInt(common.renderPatchSize(common.max_res_scale_exponent))) >=
                @as(f64, @floatFromInt(window.height)) / 2.0) common.background_needs_render = true;
            if (@abs(offset.x) * @as(f64, @floatFromInt(common.renderPatchSize(common.max_res_scale_exponent))) >=
